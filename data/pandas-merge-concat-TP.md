...
```{code-cell} ipython3
dd=pd.merge(df1, df2, left_index=True, right_on='name')
dd
```
...
```{code-cell} ipython3
m1 = pd.read_csv("multi1.csv")
m1
```

```{code-cell} ipython3
m2 = pd.read_csv("multi2.csv")
m2
```

```{code-cell} ipython3
m3 = pd.read_csv("multi3.csv")
m3
```

```{code-cell} ipython3
m1=m1.set_index(['prenom','nom']) #on doit faire une double indexation car il existe des doublons
mm=m1.merge(m2,left_index=True,right_on=['first_name','last_name'])
mm
```

```{code-cell} ipython3
mmm=pd.concat([mm.set_index(['first_name','last_name']),m3.set_index(['first','last'])])
mmm
```
